package com.axis.rtspclient {
  ...
  public class RTSPClient extends NetStreamClient implements IClient {
    ...
    private var handle:IRTSPHandle;
	 ...
	 private var setupurl_1 = "";
     private var setupurl_2 = "";
	...
	private var datacopy:ByteArray = new ByteArray();
    ...
    private function onData(event:Event):void {
      ...
      switch(data[0]) {
        ...
        default:
		//Handle RTP PACKET IN CASE OF INTERLEVED PACKET NOT FOUND
		datacopy.clear();
		data.readBytes(datacopy);
		var readflag=datacopy.length;
		data.clear();
		for(i=0;i<datacopy.length;i++)
		{
			if(datacopy[i].toString(16)==24 && datacopy[i+1].toString(16)==0)
			{
				
				data.writeBytes(datacopy,i,(datacopy.length)-i);
				data.position=0;
				break;
			}		
		}
		if(i==readflag || i==readflag-1)
		{
    ...
    private function onRTSPCommand():void {
      ...
      case STATE_PLAY:
		...
		// added following rtp-info 
        parsed.headers['rtp-info'] = 'url=' + setupurl_1 + ';seq=0;rtptime=0' + ","  + 'url=' + setupurl_2 + ';seq=0;rtptime=0';
        parsed.headers['rtcp-interval'] = 250;
        parsed.headers['range'] = 'npt=0-';
